  ...
  <div class="row">
    <div class="col-xs-12 clearfix">
      <div class=" row form-group">
        {{ Form::open(['route' => 'sales-index', 'method' => 'post', 'class' => '', 'id' => 'form-search']) }}
            ...
            <div class="col-md-1 col-xs-6 mrgb">
            <a href="{{ route('sales.index') }}" id="cancel-btn" class="form-control btn btn-warning btn-block-xs">Reset</a>
            </div>
        {{ Form::close() }}
        <div class="col-md-2 col-xs-6 mrgb" >
          {{ Form::open(['route' => ['sales-delete'], 'method' => 'post', 'id' => 'multi_delete_form', 'class' => '']) }}
          ...
          <table class="table table-bordered table-hover">
            <thead>
              <tr>
                ...
                <th>Company</th>
                <th>sgst Persentage</th>
                <th>cgst Persentage</th>
                <th>igst Persentage</th>
                <th>Action</th>
              </tr>
            </thead>
            <tbody>
              ...
              if (!empty($datums)) {
                  foreach ($datums as $data) {
                      ?>
                    <tr>
                      <td>{{ Form::checkbox('single_check',$data->sales_id,null, array('id'=>'single_check','class'=>'single_check')) }}</td>
                      <td>{{ $data->invoice_no }}</td>
                      <td>{{ $data->invoice_date }}</td>
                      <td>{{ $data->despatch_doc }}</td>
                      <td>{{ $data->challan_no }}</td>
                      <td>{{ $data->despatch_through }}</td>
                      <td>{{ $data->company_name }}</td>
                      <td>{{ $data->sgst_persentage }}</td>
                      <td>{{ $data->cgst_persentage }}</td>
                      <td>{{ $data->igst_persentage }}</td>
                      <td>
                      <a style="margin-right: 10px; font-size: 16px;" href="{{ route('sales.edit',  $data->sales_id) }}" title="Edit">
                           <i class="fa fa-edit"></i>
                      </a>
                      <a style="font-size: 16px;" href="#myModal" data-toggle="modal"  title="Delete">
                        <i class="fa fa-trash"> </i>
                      </a>
                      {{ Form::open(['route' => ['sales.destroy', $data->sales_id], 'class' => 'form-horizontal', 'id' => "delete-form" ]) }}
